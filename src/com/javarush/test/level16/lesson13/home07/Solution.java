...
public class Solution {
    ...
    public static class Gamer extends Thread {
        ...
        @Override
        public void run() { // написано мной
            int idx =0;
            String winnerName = "";
            try
            {
                while (true){
                    OnlineGame.isWinnerFound = idx == OnlineGame.steps.size();
                    if (OnlineGame.isWinnerFound) {
                        winnerName = getName();
                        System.out.println(winnerName + ":" + "победитель!");
                        return;
                    }
                    else
                    {
                    System.out.println(getName() + ":"+ OnlineGame.steps.get(idx++));
                    Thread.sleep((long) 1000/rating);
                    }
                }
            }
            catch (InterruptedException ex){
                if (!getName().equals(winnerName))System.out.println(getName() + ":" + "проиграл");
            }
        }
    }
}
